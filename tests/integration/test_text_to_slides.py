...
@pytest.mark.asyncio
async def test_text_to_slides_workflow(
    monkeypatch, sample_paper_content, sample_slide_content, tmp_path
):

    # generate_slide_content��ï
    mock_generate_slide = AsyncMock(return_value=sample_slide_content)
    monkeypatch.setattr(
        "app.services.llm_service.generate_slide_content", mock_generate_slide
    )
    ...
    slides_content = await generate_slide_content(
        paper_content=sample_paper_content, max_slides=10
    )
    ...
    if not sample_paper_content.get("images"):
        assert mock_presentation.slides.add_slide.call_count == len(
            sample_slide_content
        )
    else:
        assert (
            mock_presentation.slides.add_slide.call_count
            == len(sample_slide_content) + 1
        )
