...
async def generate_presentation(
    ...
    slides_content = await generate_slide_content(
        paper_content=paper_content, max_slides=max_slides, focus_areas=focus_areas
    )
...
def extract_text_for_slides(paper_content: Dict[str, Any]) -> Dict[str, str]:
    ...
    if sections:
        for section in sections:
            ...
            if "abstract" in heading:
                ...
                term in heading for term in ["conclusion", "summary", "future work"]
            ):
    ...
    if not sections or all(v == "" for v in result.values()):
        ...
        method_match = re.search(
            r"(?:method|methodology|materials and methods|approach|experiment)[:\s]+(.+?)(?=\n\n\d|\n\n[A-Z]|\Z)",
            full_text,
            re.IGNORECASE | re.DOTALL,
        )
        if method_match:
        ...
        results_match = re.search(
            r"(?:results|findings|observations)[:\s]+(.+?)(?=\n\n\d|\n\n[A-Z]|\Z)",
            full_text,
            re.IGNORECASE | re.DOTALL,
        )
        if results_match:
        ...
        conclusion_match = re.search(
            r"(?:conclusion|conclusions|summary|future work)[:\s]+(.+?)(?=\n\n\d|\n\n[A-Z]|\Z)",
            full_text,
            re.IGNORECASE | re.DOTALL,
        )
        if conclusion_match:
