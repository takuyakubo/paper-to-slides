...
class SummaryRequest(BaseModel):
    ...
    max_length: Optional[int] = Field(
        500, description="Maximum length of the summary in words"
    )
    style: Optional[str] = Field(
        "academic", description="Style of the summary (academic, simple, bullet_points)"
    )
    focus_areas: Optional[List[str]] = Field(
        None, description="Specific areas to focus on in the summary"
    )
...
class SummaryResponse(BaseModel):
    ...
    focus_areas: Optional[List[str]] = Field(
        None, description="Areas that were focused on"
    )
...
class KeyPoint(BaseModel):
    ...
    source_section: Optional[str] = Field(
        None, description="Section from which this was extracted"
    )
...
class KeyPointsRequest(BaseModel):
    ...
    categories: Optional[List[str]] = Field(
        None, description="Categories of key points to extract"
    )
...
class KeyPointsResponse(BaseModel):
    ...
    paper_id: str = Field(
        ..., description="ID of the paper from which key points were extracted"
    )
    key_points: List[KeyPoint] = Field(..., description="Extracted key points")
    categories: Optional[List[str]] = Field(
        None, description="Categories that were included"
    )
...
class LLMConfig(BaseModel):
    ...
    api_key: Optional[str] = Field(
        None, description="API key (normally from environment)"
    )
    other_params: Optional[Dict[str, Any]] = Field(
        None, description="Other provider-specific parameters"
    )
