...
class SlideTemplate(BaseModel):
    ...
    preview_image: Optional[str] = Field(
        None, description="Path to preview image for the template"
    )
...
class SlideRequest(BaseModel):
    ...
    include_images: bool = Field(
        True, description="Whether to include images from the paper"
    )
    max_slides: Optional[int] = Field(
        None, description="Maximum number of slides to generate"
    )
    custom_options: Optional[Dict[str, Any]] = Field(
        None, description="Custom options for slide generation"
    )
...
class SlideInfo(BaseModel):
    ...
    paper_id: str = Field(
        ..., description="ID of the paper the slides were generated from"
    )
    request_time: str = Field(..., description="Time when the slides were requested")
    status: str = Field(
        ..., description="Status of slide generation (generating, completed, error)"
    )
    ...
    completion_time: Optional[str] = Field(
        None, description="Time when slide generation completed"
    )
    output_file: Optional[str] = Field(
        None, description="Filename of the generated slides"
    )
