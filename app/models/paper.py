...
class PaperMetadata(BaseModel):
    ...
    keywords: Optional[str] = Field(
        None, description="Keywords associated with the paper"
    )
...
class PaperSection(BaseModel):
    ...
    level: int = Field(
        ..., description="Heading level (1 for main headings, 2 for subheadings, etc.)"
    )
    content: str = Field(..., description="Section content")
    start_page: Optional[int] = Field(
        None, description="Page number where section starts"
    )
...
class PaperImage(BaseModel):
    ...
    caption: Optional[str] = Field(
        None, description="Caption of the image, if available"
    )
...
class PaperContent(BaseModel):
    ...
    sections: List[PaperSection] = Field(
        default_factory=list, description="Sections of the paper"
    )
    images: List[str] = Field(
        default_factory=list, description="Paths to images extracted from the paper"
    )
    tables: Optional[List[Dict[str, Any]]] = Field(
        default_factory=list, description="Tables extracted from the paper"
    )
    references: Optional[List[str]] = Field(
        default_factory=list, description="References cited in the paper"
    )
...
class PaperInfo(BaseModel):
    ...
    status: str = Field(
        ..., description="Processing status (processing, processed, error)"
    )
    title: Optional[str] = Field(None, description="Title of the paper, if extracted")
    author: Optional[str] = Field(
        None, description="Author(s) of the paper, if extracted"
    )
    subject: Optional[str] = Field(
        None, description="Subject or category, if extracted"
    )
