...
async def get_slide_info(slide_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving slide info: {str(e)}"
        )
...
async def download_slides(slide_id: str):
    ...
    try:
        ...
        if metadata.get("status") != "completed":
            raise HTTPException(
                status_code=400, detail="Slides generation not yet complete"
            )
        ...
        if not output_file:
            raise HTTPException(
                status_code=500, detail="Output file not found in metadata"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving slides: {str(e)}"
        )
