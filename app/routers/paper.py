...
async def get_paper_info(paper_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving paper info: {str(e)}"
        )
...
async def get_paper_content(paper_id: str):
    ...
    if not content_path.exists():
        raise HTTPException(
            status_code=404, detail="Content not yet extracted or processing failed"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving paper content: {str(e)}"
        )
